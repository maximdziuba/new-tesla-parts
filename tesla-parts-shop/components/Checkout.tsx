...
const Checkout: React.FC<CheckoutProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isDifferent =
      currentDefaultAddress !== selectedAddressString &&
  ...
  const finalTotalDisplayAmount =
    currency === Currency.UAH ? finalTotalUAH : finalTotalUSD;
  const discountDisplayAmount =
    currency === Currency.UAH
      ? discountAmountUSD * effectiveRate
      : discountAmountUSD;
          ...
          >
            ...
            <section className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
                <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-sm font-bold text-gray-600 dark:text-slate-300">
                  1
                </div>
                Контактні дані
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-md p-2 focus:ring-2 focus:ring-blue-600 outline-none transition-colors"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
            ...
            <section className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
                ...
                Доставка{" "}
                <span className="text-red-500 dark:text-red-400 font-bold ml-2 text-sm flex items-center gap-1">
            ...
            <section className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
              ...
              <div className="space-y-3">
                <label
                  className={`flex items-center p-4 border rounded-lg cursor-pointer transition ${paymentMethod === PaymentMethod.IBAN ? "border-blue-600 bg-blue-50/50 dark:bg-blue-900/20" : "border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                >
                  <input
                    type="radio"
                    name="payment"
                ...
                <label
                  className={`flex items-center p-4 border rounded-lg cursor-pointer transition ${paymentMethod === PaymentMethod.COD ? "border-blue-600 bg-blue-50/50 dark:bg-blue-900/20" : "border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50"}`}
                >
                  <input
                    type="radio"
                    name="payment"
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="flex gap-3 text-sm">
                  ...
                  <div className="flex-1">
                    <div className="font-medium line-clamp-2 dark:text-slate-200">
                      {item.name}
                    </div>
            ...
            <div className="border-t border-gray-100 dark:border-slate-700 pt-4 space-y-2 mb-6">
              ...
              {discountValue > 0 && (
                <div className="flex justify-between text-blue-600 dark:text-blue-400 font-semibold animate-in fade-in duration-200">
                  <span>
                    {(() => {
                      const prefix = appliedPromoCode
                        ? `Промокод (${appliedPromoCode})`
                        : "Персональна знижка";
                      if (discountType === "percent")
                        return `${prefix} (${discountValue}%)`;
                      if (discountType === "usd")
                        return `${prefix} ($${discountValue})`;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 dark:border-slate-700 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-gray-600 dark:text-slate-300 text-sm leading-relaxed mb-6 text-center">
              ...
              Створіть особистий кабінет! Зареєстровані користувачі завжди
              отримують персональні знижки на наступні покупки та оперативні
