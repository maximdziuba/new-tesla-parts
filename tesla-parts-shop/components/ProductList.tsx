...
const ProductList: React.FC<ProductListProps> = ({ products, currency, uahPerUsd, onAddToCart, onProductClick, title }) => {
  ...
  return (
    <div className="py-8">
      {title && <h2 className="text-2xl font-bold mb-6 text-slate-900 border-l-4 border-blue-600 pl-4">{title}</h2>}
        ...
        {products.map((product) => (
          ...
          >
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              {/* ЗМІНА 5: active:text-blue-600 (моб) та xl:group-hover:text-blue-600 (ПК) */}
              <h3 className="font-semibold text-gray-900 mb-2 line-clamp-2 min-h-[3rem] active:text-blue-600 xl:group-hover:text-blue-600 transition-colors">
                {product.name}
              </h3>
              ...
              <div className="mt-auto pt-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className={`p-2 rounded-full transition ${product.inStock
                    ? 'bg-slate-50 text-slate-900 active:bg-blue-600 active:text-white xl:hover:bg-blue-600 xl:hover:text-white'
