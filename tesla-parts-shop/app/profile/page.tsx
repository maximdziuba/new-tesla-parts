            ...
            <div className="bg-white dark:bg-slate-800 shadow rounded-xl overflow-hidden border border-slate-100 dark:border-slate-700 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                {!isEditing && (
                  <button
                    onClick={() => {
                      ...
                      setEditAddress(
                        formatAddress(customerInfo?.default_address),
                      );
                      setIsEditing(true);
                    }}
              ...
              {isEditing ? (
                <form
                  ...
                  className="px-6 py-8 md:p-8 space-y-6"
                >
                  {errorMsg && (
                      ...
                      <div className="mb-2">
                        {editAddress && (
                          <div className="text-sm text-slate-600 dark:text-slate-400 mb-2 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                            <strong>Поточна адреса:</strong>{" "}
                            {formatAddress(editAddress)}
                          </div>
                        )}
                      </div>
                      <NovaPostWidget
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex items-start gap-4 p-5 bg-slate-50 dark:bg-slate-700/30 rounded-xl border border-slate-100 dark:border-slate-700/50">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-slate-900 dark:text-white font-semibold truncate text-lg">
                          {customerInfo?.email || "Не вказано"}
                        </p>
                      </div>
                    </div>
                    ...
                    {(customerInfo?.discount_value !== undefined
                      ? customerInfo.discount_value
                      : customerInfo?.discount_percent) > 0 && (
                      <div className="flex items-start gap-4 p-5 bg-blue-50/50 dark:bg-blue-950/20 rounded-xl border border-blue-100/50 dark:border-blue-900/30 animate-in zoom-in duration-200">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-blue-600 dark:text-blue-400 uppercase font-extrabold tracking-wider mb-1">
                            Персональна знижка
                          </p>
                          <p className="text-blue-950 dark:text-blue-100 font-extrabold text-2xl">
                            {(() => {
                              const val =
                                customerInfo.discount_value !== undefined
                                  ? customerInfo.discount_value
                                  : customerInfo.discount_percent;
                              const dtype =
                                customerInfo.discount_type || "percent";
              ...
              {orders.length === 0 ? (
                <div className="bg-white dark:bg-slate-800 shadow rounded-xl p-16 text-center border border-slate-100 dark:border-slate-700">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-xs mx-auto">
                    Зробіть своє перше замовлення і воно з'явиться тут для
                    відстеження статусу.
                  </p>
                  <button
                    onClick={() => router.push("/")}
                ...
                orders.map((order) => (
                  <div
                    key={order.id}
                    className="bg-white dark:bg-slate-800 shadow rounded-xl overflow-hidden border border-slate-100 dark:border-slate-700 hover:shadow-md transition duration-300"
                  >
                    <div className="p-5 md:p-8">
                      <div className="flex flex-wrap items-center justify-between gap-6 mb-8">
                        ...
                        <div className="text-right">
                          <p className="text-slate-500 dark:text-slate-400 text-sm mb-1 font-medium">
                            Сума до сплати
                          </p>
                      ...
                      <div className="bg-slate-50 dark:bg-slate-700/20 rounded-xl p-6 border border-slate-100 dark:border-slate-700/50">
                        <h5 className="text-xs uppercase font-bold tracking-widest text-slate-400 dark:text-slate-500 mb-4">
                          Склад замовлення
                        </h5>
                        <div className="space-y-4">
                          {order.items.map((item, idx) => (
                            <div
                              key={idx}
                              className="flex justify-between items-start text-sm md:text-base gap-4"
                            >
                              <div className="flex items-start gap-3">
                                <span className="bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-bold text-xs mt-0.5 flex-shrink-0">
                                  {item.quantity}x
                                </span>
                                <Link
                                ...
                                {formatCurrency(
                                  currency === Currency.UAH
                                    ? item.price_at_purchase *
                                        (order.totalUAH / (order.totalUSD || 1))
                                    : item.price_at_purchase,
                                  currency,
                                )}
                              </span>
                      ...
                      {order.ttn && (
                        <div className="mt-6 flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-bold text-blue-600/60 dark:text-blue-400/60 uppercase tracking-tighter">
                                ТТН Нова Пошта
                              </p>
                              <p className="text-blue-900 dark:text-blue-200 font-mono text-lg tracking-wider font-bold">
                                {order.ttn}
                              </p>
                            </div>
                          </div>
