...
import { Search, Percent, Eye, Check, X, ShieldAlert, ShieldCheck, Mail, Phone, Calendar, ShoppingBag } from 'lucide-react';
...
export const CustomerList: React.FC = () => {
  ...
  const [editingDiscountType, setEditingDiscountType] = useState<'percent' | 'usd' | 'uah'>('percent');
  ...
  const handleEditDiscountClick = (customer: Customer) => {
    ...
    setEditingDiscountValue((customer.discount_value !== undefined ? customer.discount_value : customer.discount_percent).toString());
  };
  ...
  const handleSaveDiscount = async (customerId: number) => {
    ...
    try {
      await ApiService.updateCustomerDiscount(customerId, editingDiscountType, value);
      setCustomers(prev => prev.map(c => 
        c.id === customerId ? { ...c, discount_type: editingDiscountType, discount_value: value, discount_percent: editingDiscountType === 'percent' ? value : 0.0 } : c
      ));
  ...
  if (loading) return <div className="p-8 text-center text-gray-500 font-medium">Завантаження клієнтів...</div>;
                ...
                filteredCustomers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="px-6 py-5">
                      ...
                      <div className="text-xs text-gray-400 font-bold mt-0.5">ID: #{customer.id}</div>
                    </td>
                    <td className="px-6 py-5">
                      ...
                      {editingDiscountId === customer.id ? (
                        <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                          <select
                            ...
                            onChange={(e) => setEditingDiscountType(e.target.value as any)}
                          >
                          ...
                          <input
                            ...
                            min={0}
                            max={editingDiscountType === 'percent' ? 100 : undefined}
                            autoFocus
                          />
                          <button
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`inline-block px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                            (customer.discount_value !== undefined ? customer.discount_value : customer.discount_percent) > 0
                              ? 'bg-blue-600 text-white shadow-sm shadow-blue-100'
                              : 'bg-gray-100 text-gray-500'
                          }`}>
                            {(() => {
                              const val = customer.discount_value !== undefined ? customer.discount_value : customer.discount_percent;
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-4xl w-full max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-slate-900">
                  Замовлення користувача: {selectedCustomer.first_name || selectedCustomer.last_name ? `${selectedCustomer.first_name || ''} ${selectedCustomer.last_name || ''}`.trim() : selectedCustomer.email}
                </h3>
                <p className="text-xs text-gray-400 mt-1">Перегляд замовлень та покупок у системі</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              {loadingOrders ? (
                <div className="py-12 text-center text-gray-500 font-medium">Завантаження замовлень...</div>
              ) : customerOrders.length === 0 ? (
                <div className="py-12 text-center text-gray-400 space-y-3">
                  <div className="mx-auto w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center">
                    <ShoppingBag size={24} className="text-slate-300" />
                  </div>
                  <p className="font-medium text-slate-500">Замовлень не знайдено</p>
                  <p className="text-xs text-gray-400">Цей користувач ще не робив покупок у нашому магазині</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {customerOrders.map((order) => {
                    ...
                    return (
                      <div key={order.id} className="border border-gray-100 rounded-xl overflow-hidden hover:border-blue-100 transition shadow-sm">
                        <div className="p-4 bg-gray-50 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            <span className="font-bold text-slate-800 text-sm">Замовлення #{order.id}</span>
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${statusDisplay.className}`}>
                              {statusDisplay.text}
                            </span>
                            {order.ttn && (
                          ...
                          <div className="space-y-3">
                            {order.items.map((item, idx) => (
                              <div key={idx} className="flex justify-between items-center text-xs md:text-sm">
                                <div className="flex items-center gap-2">
                                  <span className="bg-slate-100 px-2 py-0.5 rounded border border-slate-200 text-slate-600 font-bold text-[10px]">{item.quantity}x</span>
                                  <span className="text-slate-700 font-medium truncate max-w-sm md:max-w-lg">{item.product_name || "Товар вилучено"}</span>
                                </div>
                                <span className="text-slate-800 font-bold">${item.price_at_purchase}</span>
                              </div>
                            ))}
                            <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                              <span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Сума замовлення</span>
                              <span className="font-bold text-slate-900 text-base">${order.totalUSD}</span>
                            </div>
                          </div>
                        </div>
                      </div>
