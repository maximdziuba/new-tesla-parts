    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div className="flex items-center gap-3 mb-8">
                ...
                <h2 className="text-xl font-black uppercase tracking-widest text-slate-900">Керування сторінками</h2>
            </div>

            <div className="space-y-6">
                {pages.map(page => (
                    <div key={page.id} className="border border-gray-100 rounded-2xl p-6 bg-gray-50/30 hover:bg-gray-50 transition-all group">
                        {editingPage?.id === page.id ? (
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Заголовок сторінки</label>
                                        <input
                                            type="text"
                                        ...
                                        <label className="flex items-center gap-3 cursor-pointer group/check">
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="sr-only"
                                                />
                                                <div className={`w-10 h-6 rounded-full transition-colors ${editingPage.is_published ? 'bg-emerald-400' : 'bg-gray-300'}`}></div>
                                                <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${editingPage.is_published ? 'translate-x-4' : ''}`}></div>
                                            </div>
                                            <span className="text-xs font-bold text-slate-600 uppercase tracking-tight">Опубліковано на сайті</span>
                                        </label>
                                    </div>
                                </div>
                                
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Контент (Text/HTML)</label>
                                    <textarea
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-900 text-lg group-hover:text-blue-600 transition-colors">{page.title}</h3>
                                        <div className="flex items-center gap-3 mt-1">
                                            <span className="text-[10px] font-mono font-bold text-gray-400">/{page.slug}</span>
                                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${page.is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                ...
                {pages.length === 0 && !isCreating && (
                    <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <p className="text-gray-500 font-medium tracking-tight">Сторінок поки не створено</p>
                    </div>
                )}
            </div>
        </div>
