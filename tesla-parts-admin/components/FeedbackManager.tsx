...
import { Trash2, Upload, Plus, ArrowUp, ArrowDown, Image as ImageIcon, X, Loader } from 'lucide-react';
...
export const FeedbackManager: React.FC = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const maxSort = feedback.length > 0 ? Math.max(...feedback.map(f => f.sort_order)) : 0;
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-black uppercase tracking-widest text-slate-900">Керування відгуками</h2>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {feedback.map((item, index) => (
                    <div key={item.id} className="group relative bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-1 mt-1">
                                    <button
                                        onClick={() => {
                                            if (index > 0) {
                                                handleUpdateSort(item.id, feedback[index - 1].sort_order + 1);
                                            }
                                        }}
                                        disabled={index === 0}
            ...
            {feedback.length === 0 && !loading && (
                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-500 font-medium tracking-tight">Відгуків поки не додано</p>
                </div>
            )}
        </div>
    );
