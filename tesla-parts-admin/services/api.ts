...
export const ApiService = {
  ...
  getSocialLinks: async (): Promise<{ instagram: string; telegram: string; viber: string; whatsapp: string }> => {
    const res = await _authenticatedFetch(`${API_URL}/settings/social-links`, { headers: getHeaders() });
    if (!res.ok) throw new Error('Failed to fetch social links');
    return res.json();
  },
  updateSocialLinks: async (links: { instagram: string; telegram: string; viber: string; whatsapp: string }): Promise<any> => {
